...
export class ListFeedbackComponent implements OnInit {
  ...
  searchByDate(value: string) {
    ...
    this.feedbackService.findAllFeedBack(this.inputDate, this.currentPage).subscribe(next => {
      ...
      if (error.status == '404') {
        this.snackBar.showSnackbar('Không có phản hồi cho ngày được chọn', 'error')
      }
    })
  }
  ...
  openDialog(id: number) {
    let dialogRef = this.matDialog.open(DialogDetailComponent, {
      data: id,
      minHeight: "60%",
      width: "35%",
  ...
  jumpingPage(value: string) {
    ...
    if (numberValue > 0 && numberValue <= this.totalPages) {
      ...
      this.snackBar.showSnackbar('Vui lòng nhập số trang hợp lệ','error')
    }
  }
}
